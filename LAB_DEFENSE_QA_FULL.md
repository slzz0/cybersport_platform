...
Проверяемые исключения (checked) компилятор заставляет либо обработать, либо объявить через throws, а непроверяемые (unchecked) наследуются от RuntimeException и могут возникать во время выполнения без обязательной явной обработки.
...
Логирование — это автоматическая фиксация событий, ошибок и действий
...
AАспектно-ориентированное программирование (АОП/AOP) — это парадигма программирования, позволяющая отделить второстепенную логику (логирование, безопасность, транзакции) от основной бизнес-логики.
...
AspectJ — это расширение языка Java для аспектно-ориентированного программирования (АОП), позволяющее выносить сквозную функциональность (логирование, транзакции, безопасность) в отдельные модули — «аспекты»
